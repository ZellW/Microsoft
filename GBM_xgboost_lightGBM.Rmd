...
Install git for windows, cmake (3.8 or higher) and MS Build (MSbuild is not needed if Visual Studio is installed).

Run following command in Git Shell (right process but still fails):

Path to Rtools required:  https://stackoverflow.com/questions/8042136/install-r-package-from-source-without-changing-path-windows
...
IN `\GitHub\LightGBM\R-package` `lightgbm_2.0.5.tar.gz` is built.  This is the file you can use to install the lightGBM package in RStudio.


```{r runOnce, echo=FALSE}
library(devtools)
